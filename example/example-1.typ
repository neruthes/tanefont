#context [
  ...
  #let debug_mode = state("debug_mode", true)
  // #debug_mode.update(true)
  ...
  #set text(font: ("TeX Gyre Heros", "Noto Sans CJK SC"), size: 13pt, kerning: true, overhang: false, ligatures: false)
  #let font_def_demo01 = json("../build/demo01/final.json")


  = (Testing Precise Calibration)
  #box(width: 130%, height: 70mm, {
    set text(size: 41mm)
    // set text(size: 9mm)
    set text(font: "TeX Gyre Termes")
    set par(leading: 0.32em)
    let test_word = [Demo 01\ 010101] // Change this word
    place(top + left, block({
      show: show-tanefont-use-svg-font(font_dict: font_def_demo01)
      test_word
    }))
